<template>
  ...
  >
    <div class="update2">
      <label style="margin-bottom: 10px; font-weight: bold; font-size: 40px"
        >Edit Page</label
      >
      <div class="update3">
        <p class="updateform">
        ...
        <div class="updateinside">
          <div style="display: flex; flex-direction: column; width: 100%">
            ...
            <input
              ...
              v-model="newPassword"
              id="newPassword"
              :disabled="isDisabledinput || notClikable1"
              required
              title="Please Input Old Password First"
            />
            <!-- <signature/> -->
            <div class="uploadpicsignature">
              <label for="fileInput" class="uploadsignature" v-if="uploads"
        ...
        <div v-if="isNotEqual" class="notequal">
          <a class="notequal1"> Error!! </a>
          <a class="notequal2"> Not Equal to Old Password </a>
        </div>

        <div v-else-if="isEditemail" class="editwronge">
          <a class="editwronge1"> Error Alert!! </a>
          <a class="editwronge2"> Invalid {{ validation }} Input </a>
        </div>

        <div v-if="sendingOTP" class="succesfully">
          <a class="succesfully1"> Sending OTP.... </a>
        </div>

        <div class="verifyOTPS">
          <label for="otpInput1" class="Enterotp" v-if="showotp"
        ...
        <div v-if="succesful" class="succesfully">
          <a class="succesfully1"> OTP loaded successfully </a>
        </div>

        <!-- Loading verify otp -->
        <div v-if="veryOTP" class="succesfully">
          <a class="succesfully1"> Verifying OTP.... </a>
        </div>

        <!-- verryfied otp -->
        <div v-if="verifiedotp" class="verified">
          <a class="verified1"> Success Verified OTP </a>
        </div>
        ...
        <div class="buttonss">
          ...
          <button class="button no" @click="cli" v-if="isbackDisabled">
            Back
          </button>
        </div>
      </div>
...
const sendOTP = async () => {
  ...
  try {
    ...
    if (regex.test(newEmail.value) === false && newEmail.value !== "") {
      ...
      passvalids.test(newPassword.value) === false &&
      newPassword.value !== ""
    ) {
      isEditemail.value = true; // Set isEmail to true to show the error message
...
const updateProfile = () => {
  ...
  try {
    ...
    axios
      .post(`http://172.31.10.164:8000/update_account/${accountIdz}`, formData)
      .then((response) => {
...
const fetchAccounts = async () => {
  try {
    ...
    accounts.value = await response.data.filter(
      (result) => result.account_id == accountIdz
    );
