<template>
  <div class="dtr-form">
    ...
    <div class="dtr-table" v-if="!isMinimized">
      <div class="dtr-row">
        <div class="time-cell">
          <label>Morning In</label>
          <span v-if="times.amin">{{ formatTime(times.amin) }}</span>
          <button v-else @click="timeIn('amin')" :disabled="status !== 'amin'">Time In</button>
        </div>

        <div class="time-cell">
          <label>Morning Out</label>
          <span v-if="times.amout">{{ formatTime(times.amout) }}</span>
          <button v-else @click="timeOut('amout')" :disabled="status !== 'amout'">Time Out</button>
        </div>

        <div class="time-cell">
          <label>Afternoon In</label>
          <span v-if="times.pmin">{{ formatTime(times.pmin) }}</span>
          <button v-else @click="timeIn('pmin')" :disabled="status !== 'pmin'">Time In</button>
        </div>

        <div class="time-cell">
          <label>Afternoon Out</label>
          <span v-if="times.pmout">{{ formatTime(times.pmout) }}</span>
          <button v-else @click="timeOut('pmout')" :disabled="status !== 'pmout'">Time Out</button>
        </div>
      </div>
    </div>
