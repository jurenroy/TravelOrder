...
export default {
  data() {
    return {
      data: [],
      columns: [
        "period", "days", "rate", "premium_rate", "rate_total", "premium_total", "premium",
        "additional", "subtotal", "overpayment", "penalty", "pass_slip", "tax", "PhilHealth",
        "PhilHealthDiff", "Pagibig", "MP2", "MPL", "MGB_coop_loan", "total_deductions", "total",
        "certify", "certiby", "certipos"
      ],
      calculatedColumns: ["rate_total", "premium_total", "premium", "subtotal", "total_deductions", "total"],
      columnMapping: {
        "period": "Period", "days": "Days Worked", "rate": "Rate", "premium_rate": "Premium Rate",
        "rate_total": "Rate Total", "premium_total": "Premium Total", "premium": "Premium",
        "additional": "Additional", "subtotal": "Subtotal", "overpayment": "Over Payment",
        "penalty": "Penalty", "pass_slip": "Pass Slip", "tax": "Tax", "PhilHealth": "PhilHealth",
        "PhilHealthDiff": "PhilHealth Diff", "Pagibig": "Pagibig", "MP2": "MP2", "MPL": "MPL",
        "MGB_coop_loan": "MGB Coop Loan", "total_deductions": "Total Deductions",
        "total": "Total", "certify": "Certified By", "certiby": "Certify By ID", "certipos": "Certification Type"
      },
  ...
  methods: {
    async fetchData() {
      try {
        const res = await axios.get("http://172.31.10.43:8011/payslip-cos");
    ...
    calculate(row, col) {
      ...
      const total_deductions =
        (row.overpayment || 0) + (row.penalty || 0) + (row.pass_slip || 0) + (row.tax || 0) +
        (row.PhilHealth || 0) + (row.PhilHealthDiff || 0) + (row.Pagibig || 0) +
        (row.MP2 || 0) + (row.MPL || 0) + (row.MGB_coop_loan || 0);
      const total = subtotal - total_deductions;

      switch (col) {
        case "rate_total": return rate_total;
        case "premium_total": return premium_total;
  ...
  if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Enter", "Tab"].includes(e.key)) {
    ...
    switch (e.key) {
      case "ArrowUp": newRow = Math.max(rowIndex - 1, 0); break;
      case "ArrowDown": newRow = Math.min(rowIndex + 1, rowCount - 1); break;
      case "ArrowLeft": newCol = Math.max(colIndex - 1, 0); break;
      case "ArrowRight": newCol = Math.min(colIndex + 1, colCount - 1); break;
      case "Enter": newRow = Math.min(rowIndex + 1, rowCount - 1); break;
      case "Tab":
