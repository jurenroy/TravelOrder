    ...
    <div class="first">
      ...
      <div class="second">
        ...
        <div class="inside">
          <div style="display: flex; flex-direction: column; width: 100%">
            ...
            <select
              ...
              class="regsinput"
              id="namein"
              style="height: 35px; border: 2px solid black; width: 93%"
              required
              @click="disablenames"
            ...
            <input
              ...
              id="password"
              required
              :class="{ 'red-border': isRed && password === '' }"
              @input="resetRed"
              @keydown.enter="regis_submit"
            />
        ...
        <div v-else-if="isEmail" class="wronge">
          ...
          <a class="wronge2"> Wrong Input {{ valid }} </a>
        ...
        <div v-else-if="pleaseWait" class="correct">
          <a class="correct1"> Account Registered!! </a>
        </div>

        <div class="buttonss">
          <button class="button re" :disabled="submit2" @click="regis_submit">
            Register
          </button>
          ...
          <button class="button bax" :disabled="submit2" @click="backButton">
            Back
          </button>
        </div>
      </div>
...
export default {
  ...
  watch: {
    account_type(newVal) {
      if (newVal === 3) {
        const employeeNameIds = this.employee.map(
          (employees) => employees.name_id
        );
        this.namez = this.names.filter((name) =>
          employeeNameIds.includes(name.name_id)
        );
      } else {
    ...
    regis_submit() {
      ...
      if (
        ...
        setTimeout(() => {
          ...
          axios
            .post("http://172.31.10.164:8000/add_account/", formData)
            .then((response) => {
