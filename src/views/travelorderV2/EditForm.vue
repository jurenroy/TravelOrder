  ...
  export default {
    ...
    methods: {
      ...
      fetchSelectedEmployee() {
        ...
        if (this.selectedName && this.employees) {
      ...
      fetchData() {
        ...
        fetch(`${API_BASE_URL}/get_names_json/`)
          ...
          .then(data => {
            this.names = data;
            if (this.names) {
              if (this.authStore.account_type == 1) {
              } else if (this.authStore.account_type !== 1) {
                this.names = this.names.filter(nem => parseInt(nem.name_id) === parseInt(this.authStore.name_id))
              }
            }
          })
          .catch(error => {
      ...
      axios.get(`${API_BASE_URL}/get_forms_json/${this.travelOrderId}`)
        .then(response => {
          const travelOrders = response.data; // Assuming the response data is an array
          if (travelOrders) {
            this.populateFormData(travelOrders);
          } else {
