  ...
  fetchData(reset = false) {
    ...
    axios.get(`${API_BASE_URL}/get_forms_json/${this.nameId}/${this.selectedStatus}/${this.numberOfRows}/${this.formData.length}`)
      .then(response => {
        ...
        const existingIds = new Set(this.formData.map(item => item.travel_order_id));
    
        // Filter out duplicates
        const uniqueNewData = response.data.filter(item => !existingIds.has(item.travel_order_id));
