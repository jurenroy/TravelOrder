<template>
  ...
  <div class="a4-container" id="pdf-content">
    <div ref="content" class="a4-content">
      ...
      <img
        src="@/components/assets/hnf.jpg"
        alt="A4-sized photo"
        class="a4-photo"
      />
      <p class="a4-to" style="text-align: center">TRAVEL ORDER</p>
      <p class="a4-textBold" style="margin-top: -15px; text-align: center">
        No.
        <span style="text-decoration: underline; text-align: center"
          >{{ padWithZeroes(to_num) }}-{{ yearToday }}</span
        >
      </p>
      ...
      >
        ...
        >
          <p class="label2" style="margin-top: 35px; margin-left: 6%">
            Certifications:
          </p>
        </div>
        <div
        ...
        >
          <p
            class="label2"
            style="margin-left: 6%; text-align: justify; width: 88%"
          >
            This is to certify that the travel is necesarry and is connected
            with the function of the official/employee of this
        ...
        <div
          ...
          :style="{
            marginTop: '-30px',
            justifyContent:
              division == 'ORD' || name == 'LIBERTY B. DAITIA'
                ? 'flex-end'
                : 'space-around',
          }"
          v-if="aor == 0"
        >
          <div
            class="inner-container2"
            v-if="
              !divisionChiefs.includes(name) &&
              intervals == 0 &&
              division !== 'ORD'
            "
          >
            ...
            <img
              :src="signature1"
              class="signatiz"
              v-if="signature1 !== 'http://172.31.10.164:8000/storage/null'"
            />
            <p
              class="value"
              :style="{
                'font-weight': 'bold',
                'margin-top':
                  signature1 == 'http://172.31.10.164:8000/storage/null'
                    ? '50px'
                    : '50px',
              }"
            >
          ...
          >
            <p>Approved by:</p>
            <p style="margin-top: -15px" v-if="sname !== 20 && sname !== null">
            ...
            <img
              ...
              class="signatiz"
              v-if="
                signature2 !== 'http://172.31.10.164:8000/storage/null' &&
                aor == 0
              "
            />
            <p
            ...
            <p
              ...
              v-if="sdiv == 1 && sdiv !== null"
            >
              JANICE B.FUROG
            </p>
            <p style="margin-top: -10px" v-if="sdiv == 5 || sdiv == null">
              OIC, Regional Director
            </p>
            <p style="margin-top: -10px" v-if="sdiv !== 5 && sdiv !== null">
              Chief, {{ sdivz.division_name }}
            </p>
          </div>

          <div
            class="inner-container2"
            :style="{ marginRight: division == 'ORD' ? '90px' : '0px' }"
            v-if="name == 'RODANTE B. FELINA'"
          >
            <p>Approved by:</p>
            <img
        ...
        <div
          ...
          :style="{
            ...
            justifyContent:
              (division == 'ORD' ||
                (name == 'RODANTE B. FELINA' && intervals == 0) ||
                (name == 'LIBERTY B. DAITIA' && intervals == 0)) &&
              !(name == 'RODANTE B. FELINA' && intervals == 1)
                ? 'flex-end'
                : 'space-around',
          }"
          v-if="aor == 1"
        >
          ...
          <div
            ...
            v-if="
              ((divisionChiefs.includes(name) && intervals == 0) ||
                !divisionChiefs.includes(name)) &&
              name != 'RODANTE B. FELINA'
            "
          >
            <p>Approved by:</p>
            <p style="margin-top: -15px" v-if="sname !== 20 && sname !== null">
          ...
          <div
            class="inner-container2"
            :style="{ marginRight: division == 'ORD' ? '-10px' : '0px' }"
            v-if="divisionChiefs.includes(name) && intervals == 1"
          >
            <p>Approved by:</p>
            <img
              :src="signature2"
              class="signatiz"
              v-if="signature2 !== 'http://172.31.10.164:8000/storage/null'"
            />
            <p
              class="value"
              :style="{
        ...
        <p style="letter-spacing: 5px; text-align: center; margin-top: -25px">
          AUTHORIZATION
        </p>
        <p
          style="
          ...
          "
        >
          I hereby authorize the Accountant top deduct the corresponing amount
          of the unliquidated cash advance from my succeeding salary for my
          failure to liquidate this travel within twenty(20) days upon return to
          my permanent official station pursuant to Commision on Audit(COA)
          Circular No. 2012-004 dated November 28, 2012.
        </p>
        <div
          class="inner-container"
          style="align-self: flex-end; margin-right: 6%"
        >
          <img
            :src="signature3"
            class="signatizz"
            v-if="signature3 !== 'http://172.31.10.164:8000/storage/null'"
        ...
        <img
          ...
          "
        />
        <p style="text-align: left; margin-top: -60px; margin-left: 6%">
          MGB-X-FAD-FO-033
        </p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    populateFields(travelOrderId) {
      const selectedForm = this.formData.find(
        (form) => form.travel_order_id === travelOrderId
      );
      if (selectedForm) {
        ...
        const selectedPosition = this.positions.find(
          (position) => position.position_id === this.position_id
        );
        if (selectedPosition) {
          this.position = selectedPosition.position_name;
        }

        this.sdivz = this.divisions.find(
          (division) => division.division_id === this.sdiv
        );

        const selectedDivision = this.divisions.find(
          (division) => division.division_id === this.division_id
        );
        if (selectedDivision) {
          this.division = selectedDivision.division_name;
          const recommendedEmployee = this.employees.find(
            (employee) =>
              employee.division_id === this.division_id && employee.chief > 0
          );
        ...
        const selectedAccount = this.accounts.find(
          (acc) => acc.name_id === nameDetails.name_id
        );
        if (selectedAccount) {
