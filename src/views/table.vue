  ...
  <div style="display: flex; flex-direction: column">
    ...
    <div class="note" v-if="viewNote">
      ...
      <div class="content">
        ...
        <div class="butokz">
          <button @click="postNote" v-if="siga || siga1 || acc.name_id == 76">
            Save
          </button>
    ...
    <div style="display: flex; flex-direction: column; margin-top: -60px">
      ...
      >
        ...
        <button
          v-if="mawala && [2, 15, 27, 76, 39].includes(acc.name_id)"
          class="Btn"
          @click="downloadCSV"
        >
          <div class="sign">
              ...
              <tr v-for="item in reversedFormData" :key="item.id">
                ...
                <td v-else style="color: green">
                  <p
                    v-if="
                      ![
                        39, 2, 3, 8, 42, 34, 29, 36, 48, 5, 47, 15, 45, 21, 52,
                        51, 13, 10, 37, 62, 53, 75, 4, 56, 58, 55, 60, 59, 20,
                      ].includes(item.name_id) && item.initial !== null
                    "
                  ...
                  >
                    ...
                    />
                    {{
                      item.initial.charAt(0).toUpperCase() +
                      item.initial.slice(1)
                    }}
                  </p>

                  <p
                    v-if="item.note === null"
                    style="color: red; margin-top: 1px; margin-bottom: -15px"
                  >
                    <img
                      src="../assets/close.png"
                      style="height: 10px; width: 10px"
                  ...
                  <p
                    v-else
                    style="color: green; margin-top: 2px; margin-bottom: -15px"
                  >
                    <img
                      src="../assets/check.png"
                      style="height: 10px; width: 10px"
                  ...
                  <p
                    v-if="
                      (item.signature1 === null &&
                        item.note !== null &&
                        ![
                          15, 20, 21, 45, 48, 13, 10, 37, 62, 53, 75, 56, 58,
                          55, 60, 59,
                        ].includes(item.name_id)) ||
                      (item.signature1 === null &&
                        item.note !== null &&
                        [15, 21, 45, 48].includes(item.name_id) &&
                        item.intervals == 1)
                    "
                  ...
                  <p
                    v-if="
                      (item.signature2 === null && item.signature1 !== null) ||
                      ([
                        15, 20, 21, 45, 48, 13, 10, 37, 62, 53, 75, 4, 56, 58,
                        55, 60, 59,
                      ].includes(item.name_id) &&
                        item.signature2 === null &&
                        item.note !== null)
                    "
                    style="color: red"
                  >
                ...
                <td style="display: flex; justify-content: center">
                  <button
                    v-if="selectedTravelOrderId != item.travel_order_id"
                    @click="openPDF(item.travel_order_id)"
                  >
                    PDF
                  </button>
                  <img
                ...
                <td
                  v-if="
                    ...
                    item.note !== null &&
                    item.signature1 == null &&
                    item.name_id !== acc.name_id
                  "
                  style="display: flex; justify-content: center"
                >
                  <button @click="signature1(item.travel_order_id)">
                    Recommend
                  </button>
                </td>
                ...
                >
                  <button
                    @click="signature11(item.travel_order_id, item.name_id)"
                  >
                    Recommend
                  </button>
                </td>
                ...
                <td
                  v-if="
                    item.travel_order_id !== notenum &&
                    item.note == null &&
                    acc.name_id == 37
                  "
                  style="display: flex; justify-content: center"
                >
                  <button @click="openNote(item.travel_order_id)">
                    Add note
                  </button>
                </td>
                ...
                <td
                  v-if="item.note !== null && notenum !== item.travel_order_id"
                  style="display: flex; justify-content: center"
                >
                  <button @click="viewNotez(item.note, item.travel_order_id)">
                    View note
                  </button>
                </td>
                ...
                <td
                  ...
                  style="display: flex; justify-content: center"
                >
                  <button @click="signature2(item.travel_order_id)">
                    Approve
                  </button>
                </td>
                ...
                <td
                  v-if="
                    isSectionChief(acc.name_id) &&
                    selectedTravelOrderId != item.travel_order_id &&
                    item.initial === null
                  "
                  style="display: flex; justify-content: center"
                >
...
export default {
  ...
  methods: {
    ...
    downloadCSV() {
      ...
      const csvData = Object.keys(groupedByMonth).flatMap((yearMonthKey) => {
        ...
        Object.keys(yearData).forEach((yearKey) => {
          ...
          const month = yearMonthDate.toLocaleString("default", {
            month: "long",
          });
    ...
    viewNotez(nutz, numx) {
      (this.viewNote = true), (this.noteText = nutz);
      this.notenum = numx;
    },
    openNote(numz) {
      (this.addNote = true), (this.notenum = numz);
      this.noteText = "";
    },
    closeNote() {
      (this.addNote = false), (this.viewNote = false), (this.notenum = 0);
    },
    postNote() {
      ...
      axios
        .post(
          `http://172.31.10.164:8000/update_form/${this.notenum}`,
          formData,
          {
            headers: {
    ...
    initialize(numz) {
      ...
      axios
        .post(
          `http://172.31.10.164:8000/update_form/${this.initnum}`,
          formData,
          {
            headers: {
              "Content-Type": "multipart/form-data",
    ...
    async signature11(form_id, name_id) {
      ...
      axios
        .post(`http://172.31.10.164:8000/update_form/${form_id}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
    ...
    async waitForVerifiedOTPs() {
      while (this.verifiedOTPs == "false") {
        await new Promise(
          ...
          (this.verifiedOTPs = localStorage.getItem("verifiedOTPs"))
        ); // Wait for 10 seconds
      }
    },
    ...
    fetchAccounts() {
      axios
        .get("http://172.31.10.164:8000/get_accounts_json")
        .then((response) => {
    ...
    fetchData() {
      this.load = true;
      axios
        .get("http://172.31.10.164:8000/get_forms_json")
        .then((response) => {
          this.mawala = true;
          this.load = false;
          this.sub = this.employees.find(
            (emp) => emp.name_id == this.acc.name_id
          );
          ...
          if (this.sectionChiefIds.includes(this.acc.name_id)) {
            ...
            this.formData = response.data.filter(
              (form) =>
                form.name_id == this.acc.name_id ||
            ...
            this.formData = response.data.filter(
              (form) =>
                form.name_id == this.acc.name_id ||
                (form.note == null && form.initial !== null)
            );
            this.siga = false;
            ...
            this.formData = response.data.filter(
              (form) => form.name_id == this.acc.name_id
            );
            this.siga = false;
            ...
            this.formData = response.data.filter(
              (form) =>
                (form.signature2 === null &&
                  form.signature1 !== null &&
                  form.note !== null &&
                  !(
                    form.aor == 1 && [15, 21, 45, 48].includes(form.name_id)
                  )) ||
                (form.division_id === 5 &&
                  form.signature2 == null &&
                  form.note !== null) ||
                (form.division_id !== 5 &&
                  ...
                  form.note !== null &&
                  this.acc.name_id !== 20 &&
                  form.division_id == this.bus.division_id) ||
                form.name_id == 20 ||
                ([15, 21, 45, 48].includes(form.name_id) &&
                  form.aor == 1 &&
                  form.signature1 == null)
            );
            this.siga1 = true;
          } else if (this.acc.type_id == 3) {
            const division_id = this.employees.find(
              (name) => name.name_id == this.acc.name_id
            ).division_id;
            this.formData = response.data.filter(
              (form) =>
            ...
            if (this.sub.name_id == 20) {
              this.formData = response.data.filter(
                (form) => form.name_id == this.acc.name_id && form.note !== null
              );
              this.siga = false;
            }
          }
    ...
    fetchNames() {
      axios
        .get("http://172.31.10.164:8000/get_names_json")
        .then((response) => {
    ...
    fetchEmployees() {
      axios
        .get("http://172.31.10.164:8000/get_employees_json")
        .then((response) => {
    ...
    reversedFormData() {
      ...
      if (this.searchQuery !== "") {
        data = data.filter((item) => {
          // Filter based on TO number or Name
          return (
            String(this.padWithZeroes(item.to_num)).includes(
              this.searchQuery
            ) ||
