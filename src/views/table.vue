        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in reversedFormData" :key="item.id">
              ...
              <td v-if="siga && item.travel_order_id !== notenum && item.note == null && acc.name_id == 37"
                style="display: flex; justify-content: center;"><button @click="openNote(item.travel_order_id)">Add
                  note</button></td>

              <td v-if="siga && addNote == true && item.travel_order_id == notenum && acc.name_id == 37"
              ...
              <td v-if="item.note !== null && notenum == item.travel_order_id"
                style="display: flex; justify-content: center;"><button @click="closeNote()">Close View note</button>
              </td>
